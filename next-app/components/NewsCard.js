...
function FormatShare(cardData,langSel){
	...
	msg=msg+'`=========`%0A%0A';
	msg=msg+`*Shared from ${siteOrigin}* %0AA volunteer-driven platform bringing you wide-ranging news snippets on Covid-19 from prominent media of India and the world in multiple languages.%0A%0A`;
	msg=msg+`Source : ${cardData.link}`;
...
function NewsCard(props){
	...
	return(
		<div className={classNames("NewsCard",{
			...
			{/*FEATURED */}
			{props.cardData.featured&&<img className="NewsCardImageFeatured" alt="Featured" src="/assets/featured.png"/>}
			{/*Preview Image*/}
			...
			<div className={classNames("NewsCardContent",{
				...
				<div className="NewsCardRow">
					...
					<a href={FormatShare(props.cardData,langSel)}>
						<img className="NewsCardShare" alt="Share" src="/assets/whatsapp.png"/>
					</a>
					{/*Save card*/}
					<img onClick={()=>{setSaveCard(!saveCard); toggleSaved(props.cardData)}} className="NewsCardSave" alt="Share" src={`/assets/save${saveCard?"-yes":"-no"}.png`}/>
